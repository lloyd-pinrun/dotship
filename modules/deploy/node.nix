...
in {
  ...
  options = {
    ...
    profiles = dot.options.attrs.submoduleWith "profiles available for deployment to node" {inherit flake node;} ./profile.nix;
    order = dot.options.list.enum (builtins.attrNames config.profiles) "order of profiles to deploy" {};

    dotship = {
      os = dot.options.enum ["nixos" "macos" "windows" "linux"] "node operation system" {default = "nixos";};
      system = dot.options.str "node architecture" {
