...
workspace_path="$workspaces_path.$WORKSPACE.config"
tofu=("nix" "run" "$workspace_path.canivete.package" -- -chdir="$run_dir")

mkdir -p "$run_dir"
trap 'rm -rf "$run_dir/.terraform" "$run_dir/.terraform.lock.hcl" "$dec_file"' EXIT
nix build "$workspace_path.canivete.json" --no-link --print-out-paths |
    xargs cat |
    vals eval -s -f - |
    yq "." >"$dec_file"
"${tofu[@]}" init -upgrade
