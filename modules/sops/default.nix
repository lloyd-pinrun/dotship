...
in {
  ...
  perSystem = {
    ...
    config.canivete = {
      ...
      in
        pkgs.wrapProgram config.canivete.scripts.sops-encrypt.package "sops-encrypt" "sops-encrypt" args {};
      sops.setup = let
        args = "--prefix PATH : ${makeBinPath (with pkgs; [sops])}";
      in
        pkgs.wrapProgram config.canivete.scripts.sops-setup.package "sops-setup" "sops-setup" args {};
    };
  };
}
