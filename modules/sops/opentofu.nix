{
  ...
  options.dotship.opentofu.workspaces = dot.options.attrs.withSubmodule ({config, ...}: {
    options.passwords = dot.options.attrs.attrs.anything "terraform random_password configs" {};

    options.sops = dot.options.attrs.submodule "secrets generated by terraform to store in sops" ({
      config,
      name,
      ...
    }: {
      options = {
        value = dot.options.str "terraform attribute reference with sops secret contents" {};
        path = dot.options.list.str "path in the default encrypted sops file to drop the secret" {default = [name];};

        env = dot.options.str "name of variable in a shell environment" {
          default = builtins.replaceStrings ["." "-"] ["__" "_"] (lib.toUpper name);
        };

        command = dot.options.str "command to save value in sops, running in the root directory" {
          default = let
