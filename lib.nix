lib: let
  ...
  # NOTE: Fileset collectors
  filesets = rec {
    # NOTE: List absolute path of files in <root> that satisfy <fun>
    _filter = fun: root:
      lib.pipe root [
        builtins.readDir
        (lib.filterAttrs fun)
    ...
    # NOTE: Lists directories in <root>
    dirs = _filter (_: type: type == "directory");
    # NOTE: Lists files in <root> that satisfy <fun>
    files = fun: _filter (name: type: type == "regular" && fun name type);
    # NOTE: Recursively lists all files in <dirs> that satisfy <fun>
    everything = fun: let
      filesAndDirs = root:
    ...
    # NOTE: Rejects <excluded> paths from "everything" in <roots> that satisfy <fun>
    ...
    # NOTE: All of the previous functions (except for `dirs`) with a predefined filter for `nix` files
    nix = {
  ...
  # keep-sorted start
  # NOTE:
  #   Wrapped option types
  #     "attrs.str" option: `options.foo.accounts = dot.attrs.str "mapping of external accounts to foo" {};`
  #     "list.str" option: `options.foo.things = dot.list.str "list of strings for foo.things" {};`
  attrs = wrapped types.nullOr {inherit attrs function list opt;};
  function = wrapped.functionTo {inherit attrs function list opt;};
  list = wrapped types.listOf {inherit attrs function list opt;};
  opt = wrapped types.nullOr {inherit attrs function list;};
  # keep-sorted end

  # NOTE:
  #   Aggregated custom `options` functions
  #     "enable" option (`default = false`): `options.foo.enable = dot.options.enable "enable foo" {};`
  #     "enabled" option (`default = true`): `options.foo.enabled = dot.options.enable "enable foo" {};`
  #     "domain" option: `options.foo.domain = dot.options.domain "base domain of foo" {default = "foo.bar"};`
  #     "enum" option: `options.foo.type = dot.options.enum ["bar"] "the type of foo"`
  options =
    (wrapped lib.id {} {})
    // {
      attrs = attrs {default = {};};
